...
@section('content')
  ...
  <div class="tuaninfo clearfix overh bgc_f">
    ...
    <div class="ti_right overh f-l">
      ...
      <div class="ti_r_b overh bgc_sr">
        <del>原价：￥<i class="old_shop_price">{{ $good->shop_price }}</i></del> 团购价：<span class="font_md">￥{{ $timetobuy->price }}</span>
      </div>
    </div>
  </div>
  <!-- 产品信息 -->
  <section class="goodinfo clearfix mt20 bgc_f pd20">
    <h1 class="good_title">
      ...
      @endif
      <i class="label label-red">每人限购 {{ $timetobuy->buy_max }} 件</i> {{ $good->title }}</h1>
    <div class="g_i_prices mt10 clearfix">
      <span class="gi_price font_lg color_main f-l"><del>￥<i class="old_shop_price">{{ $good->shop_price }}</i></del>￥<i class="shop_price">{{ $timetobuy->price }}</i></span>
