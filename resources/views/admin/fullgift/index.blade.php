...
@if(App::make('com')->ifCan('fullgift-add'))
@section('rmenu')
	<a href="{{ url('/console/fullgift/add') }}" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-plus"></span> 添加赠品</a>
@endsection
@endif
...
<table class="table table-striped table-hover mt10">
	...
	<tr>
		...
		<td>{{ $a->endtime }}</td>
		<td>
			@if(App::make('com')->ifCan('fullgift-edit'))
			<a href="{{ url('/console/fullgift/edit',['id'=>$a->id]) }}" class="btn btn-xs btn-info glyphicon glyphicon-edit"></a>
			@endif
			@if(App::make('com')->ifCan('fullgift-del'))
			<a href="{{ url('/console/fullgift/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
			@endif
		</td>
	</tr>
	@endforeach
...
<div class="pull-left" data-toggle="buttons">
	...
	@if(App::make('com')->ifCan('fullgift-sort'))
	<span class="btn btn-xs btn-warning btn_sort">排序</span>
	@endif

	@if(App::make('com')->ifCan('fullgift-alldel'))
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/fullgift/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/console/fullgift/alldel') }}",'method':'post'}).submit();
			}
		});
