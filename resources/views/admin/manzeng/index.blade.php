...
<table class="table table-striped table-hover mt10">
	...
	<tr>
		...
		<td>
			...
			<div data-url="{{ url('/console/manzeng/edit',['id'=>$a->id]) }}" data-title="修改满赠" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-info glyphicon glyphicon-edit btn_modal"></div>
			@endif
			@if(App::make('com')->ifCan('manzeng-del'))
			<a href="{{ url('/console/manzeng/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
			@endif
		</td>
	</tr>
	@endforeach
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/manzeng/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/console/manzeng/alldel') }}",'method':'post'}).submit();
			}
		});
