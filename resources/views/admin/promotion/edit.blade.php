...
<form action="javascript:;" method="post" id="form_ajax">
    ...
    <table class="table table-striped">
        ...
        <tr>
            ...
            <td>
                <select name="data[type]" class="form-control input-sm">
                    <option value="1"@if($info->type == '1') selected @endif>折扣</option>
                    <option value="2"@if($info->type == '2') selected @endif>减价</option>
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <input type="number" min="0" name="data[type_val]" value="{{ $info->type_val }}" class="form-control input-sm">
            ...
            <td>
                <table class="table-bordered table">
                    ...
                    <tbody class="good_lists">
                        ...
                        <tr class="good_tr_{{ $g->id }}">
                            <td>{{ $g->title }}</td>
                            <td>{{ $g->price }}￥</td>
                            <td>{{ $g->store }}</td>
                            <td><input name='good_id[]' type='hidden' value="{{ $g->id }}" /><span data-id="{{ $g->id }}" class='btn_del_good btn btn-xs btn-danger glyphicon glyphicon-trash'></span></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <tr>
            ...
            <td>
                <div class="btn-group">
                    ...
                    <div onclick='ajax_submit_form("form_ajax","{{ url('/console/promotion/edit',['id'=>$info->id]) }}")' name="dosubmit" class="btn btn-xs btn-info">提交</div>
                </div>
            </td>
        </tr>
