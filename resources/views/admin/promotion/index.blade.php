...
@section('rmenu')
@if(App::make('com')->ifCan('promotion-add'))
	<a href="{{ url('/console/promotion/add') }}" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-plus"></span> 添加活动</a>
@endif
@endsection
...
<table class="table table-striped table-hover mt10">
	...
	@foreach($list as $a)
		<tr>
			...
			<td>{{ $a->endtime }}</td>
			<td>
				@if(App::make('com')->ifCan('promotion-goodlist'))
				<a href="{{ url('/console/promotion/goodlist',$a->id) }}" title="查看商品" class="btn btn-xs btn-success glyphicon glyphicon-eye-open"></a>
				@endif
				@if(App::make('com')->ifCan('promotion-edit'))
				<a href="{{ url('/console/promotion/edit',['id'=>$a->id]) }}" class="btn btn-xs btn-info glyphicon glyphicon-edit"></a>
				@endif
				@if(App::make('com')->ifCan('promotion-del'))
				<a href="{{ url('/console/promotion/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
				@endif
			</td>
		</tr>
	@endforeach
...
<div class="pull-left" data-toggle="buttons">
	...
	@if(App::make('com')->ifCan('promotion-sort'))
	<span class="btn btn-xs btn-warning btn_sort">排序</span>
	@endif

	@if(App::make('com')->ifCan('promotion-alldel'))
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/promotion/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/console/promotion/alldel') }}",'method':'post'}).submit();
			}
		});
