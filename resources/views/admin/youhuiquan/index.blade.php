...
@if(App::make('com')->ifCan('youhuiquan-add'))
	<div data-url="{{ url('/console/youhuiquan/add') }}" data-title="添加优惠券" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-default btn_modal"><span class="glyphicon glyphicon-plus"></span> 添加优惠券</div>
@endif
...
<table class="table table-striped table-hover mt10">
	...
	<tr>
		...
		<td>
			...
			<div data-url="{{ url('/console/youhuiquan/edit',['id'=>$a->id]) }}" data-title="修改优惠券" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-info glyphicon glyphicon-edit btn_modal"></div>
			@endif
			@if(App::make('com')->ifCan('youhuiquan-del'))
			<a href="{{ url('/console/youhuiquan/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
			@endif
		</td>
	</tr>
	@endforeach
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/youhuiquan/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/console/youhuiquan/alldel') }}",'method':'post'}).submit();
			}
		});
