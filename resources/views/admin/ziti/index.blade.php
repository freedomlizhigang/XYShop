...
@if(App::make('com')->ifCan('ziti-add'))
	<div data-url="{{ url('/console/ziti/add') }}" data-title="添加自提点" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-default btn_modal"><span class="glyphicon glyphicon-plus"></span> 添加自提点</div>
@endif
...
<table class="table table-striped table-hover mt10">
	...
	<tr>
		...
		<td>
			...
			<div data-url="{{ url('/console/ziti/edit',['id'=>$a->id]) }}" data-title="修改自提点" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-info glyphicon glyphicon-edit btn_modal"></div>
			@endif
			@if(App::make('com')->ifCan('ziti-del'))
			<a href="{{ url('/console/ziti/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
			@endif
		</td>
	</tr>
	@endforeach
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/ziti/sort') }}",'method':'post'}).submit();
		});
