...
@section('rmenu')
@if(App::make('com')->ifCan('coupon-add'))
	<div data-url="{{ url('/console/coupon/add') }}" data-title="添加优惠券" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-default btn_modal"><span class="glyphicon glyphicon-plus"></span> 添加优惠券</div>
@endif
...
<table class="table table-striped table-hover mt10">
	...
	<tr>
		...
		<td><span class="text-primary">{{ $a->price }}￥ - {{ $a->lessprice }}￥</span></td>
		<td>{{ $a->nums }}</td>
		<td>
			@if($a->status == 1)
		...
		<td>{{ $a->endtime }}</td>
		<td>
			@if(App::make('com')->ifCan('coupon-edit'))
			<div data-url="{{ url('/console/coupon/edit',['id'=>$a->id]) }}" data-title="修改优惠券" data-toggle='modal' data-target='#myModal' class="btn btn-xs btn-info glyphicon glyphicon-edit btn_modal"></div>
			@endif
			@if(App::make('com')->ifCan('coupon-del'))
			<a href="{{ url('/console/coupon/del',$a->id) }}" class="confirm btn btn-xs btn-danger glyphicon glyphicon-trash"></a>
			@endif
		</td>
	</tr>
	@endforeach
...
<div class="pull-left" data-toggle="buttons">
	...
	@if(App::make('com')->ifCan('coupon-sort'))
	<span class="btn btn-xs btn-warning btn_sort">排序</span>
	@endif

	@if(App::make('com')->ifCan('coupon-alldel'))
...
<script>
	$(function(){
		$('.btn_sort').click(function(){
			$('.form_submit').attr({'action':"{{ url('/console/coupon/sort') }}",'method':'post'}).submit();
		});
		$('.btn_del').click(function(){
			if (!confirm("确实要删除吗?")){
				return false;
			}else{
				$('.form_submit').attr({'action':"{{ url('/console/coupon/alldel') }}",'method':'post'}).submit();
			}
		});
